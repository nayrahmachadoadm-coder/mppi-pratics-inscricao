-- Criação da tabela inscricoes para armazenar os dados do formulário
-- Inscrição Melhores Práticas MPPI 2025
...
CREATE INDEX IF NOT EXISTS idx_inscricoes_email_institucional ON public.inscricoes(email_institucional);
CREATE INDEX IF NOT EXISTS idx_inscricoes_status ON public.inscricoes(status);
CREATE INDEX IF NOT EXISTS idx_inscricoes_area ON public.inscricoes(area);
...
-- (Para implementar futuramente quando houver sistema de autenticação)
CREATE POLICY "Permitir leitura para administradores" ON public.inscricoes
    FOR SELECT 
    USING (false); -- Por enquanto, nenhuma leitura permitida via RLS
...
CREATE POLICY "Permitir atualização para administradores" ON public.inscricoes
    FOR UPDATE 
    USING (false); -- Por enquanto, nenhuma atualização permitida via RLS
