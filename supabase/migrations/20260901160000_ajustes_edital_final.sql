-- Migration para ajustes de edital final: remoção de ODS, divisor fixo e substituição de inscrições

-- 1. Remoção do ODS e recálculo do Total
...
BEGIN
    ...
    -- ODS Removido. Apenas soma os 5 critérios.
...
-- 2. Atualizar divisor para 25.0 em ambas categorias
...
-- 3. Atualizar duplicidade para não bloquear via RPC (o Frontend avisará)
...
BEGIN
    ...
    WHERE matricula = p_matricula
      ...
      AND edicao_ano = 2026
      AND status_inscricao NOT IN ('Cancelada', 'Substituída');

    RETURN v_count > 0;
END;
$$;

-- 4. Trigger de substituição de inscrições anteriores
CREATE OR REPLACE FUNCTION public.trg_substituir_inscricoes_anteriores()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF NEW.matricula IS NOT NULL AND NEW.tipo_iniciativa IS NOT NULL THEN
        UPDATE public.inscricoes
        SET status_inscricao = 'Substituída'
        WHERE matricula = NEW.matricula 
          AND tipo_iniciativa = NEW.tipo_iniciativa
          AND edicao_ano = NEW.edicao_ano
          AND id != NEW.id
          AND status_inscricao NOT IN ('Cancelada', 'Substituída');
    END IF;
    RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_inscricoes_substituicao ON public.inscricoes;
CREATE TRIGGER trg_inscricoes_substituicao
AFTER INSERT ON public.inscricoes
FOR EACH ROW
EXECUTE FUNCTION public.trg_substituir_inscricoes_anteriores();

-- 5. Atualizar função consolidar_finalistas_v2 (sem ODS e com nova ordem de desempate)
...
BEGIN
    ...
    IF NOT p_ignorar_pendencias THEN
        FOR v_aval_count IN (
            ...
            WHERE i.status_inscricao NOT IN ('Indeferida', 'Cancelada', 'Substituída', 'Em Análise') 
            GROUP BY i.id
...
-- 6. Atualizar função apurar_resultados_finais_v2
        ...
        CASE
            WHEN apc.is_finalista THEN
                CASE
                    ...
                    WHEN media_impacto != lag_imp THEN '8.9, III (Impacto Social)'
